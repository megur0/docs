...
* Current
    * 現行のバージョン
* Upgradable
    * 互換性が保証される範囲でのアップデート可能なバージョン(flutter pub upgradeでアップデートされるバージョン)
* Resolvable
    * 各パッケージのpubspec.ymlの制約に基づいて、解決可能な最新バージョン
    * したがってこのバージョンが現状、自身のプロジェクトが利用可能な最新のバージョンとなる。
    * (IMO)マイナー・パッチバージョン単位でLatestと乖離している場合は問題がないが、メジャーバージョン単位で乖離している場合は、依存先のパッケージに問題があると考えられる。
    * このバージョンをLatestにより近づけたい場合は、
        * 該当バージョンに依存するパッケージを見つけて、
